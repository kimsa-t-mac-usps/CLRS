...
<!--- SETDOMAINCOOKIES="true" ensures CFID/CFTOKEN session cookies are scoped to the domain
     so the load balancer can enforce sticky-session affinity across all 4 nodes. --->
...
<!---<CFSET LDAPServerName = "eagandcs-sha2.usa.dce.usps.gov"> Updated to ldaps-prod 6.12.26 --->
...
<!--- Softcoded list of SIT server nodes — used for auth bypass and MFA skip --->
<CFSET Sit_Server_List = "eagnmnwbd205,eagnmnwbd206,eagnmnwbd240,eagnmnwbd1075,lawdept-sit.usps.gov,lawdept1-sit.usps.gov">
...
--->
		...
		<!--- Use CGI.HTTP_HOST (the load balancer's virtual hostname from the Host: header)
		     NOT CGI.SERVER_NAME (the individual backend node name) so all generated
		     URLs route through the load balancer, not directly to a single node. --->
		<CFSET This_Server = CGI.HTTP_HOST>
...
<!--- Centralized base URL variables — all use CGI.HTTP_HOST so links route via the load balancer --->
<CFSET This_Server_Base_URL = "https://" & This_Server>
<CFSET LawDept_Base_URL = "https://" & CGI.HTTP_HOST>
<CFSET LawDept1_Base_URL = "https://" & CGI.HTTP_HOST>
<CFSET LawManager_Base_URL = "https://" & CGI.HTTP_HOST & "/lmWeb">

<!--- Moved to LabelLists.cfm
<cfset YesNo_List = "Y,N">
--->
<cfif len(cgi.auth_user) eq 0 and ListFindNoCase(Sit_Server_List, cgi.SERVER_NAME) >
...
<cftry>
		...
		<cffinally>
			...
			<cfelse>
				<cflog text="LDAP QueryGetDisplayName: query failed or returned no results - using fallback" type="warning" file="clrs-ldap">
			</cfif>
		</cffinally>
		</cftry>
...
<cftry>
	<cfldap action="QUERY"
		...
		<cffinally>
			...
			<cfelse>
				<cflog text="LDAP QueryGetBusServContactDisplayName: query failed or returned no results - using fallback" type="warning" file="clrs-ldap">
			</cfif>
		</cffinally>
	</cftry>
	<CFIF IsDefined("QueryGetBusServContactDisplayName") AND QueryGetBusServContactDisplayName.RecordCount GT 0>
	...
	<CFELSE>
		<CFSET This_BusServContact_From_Line = '"' & Get_Bus_Serv_Contact.Trim_LASTNAME & ', ' & Get_Bus_Serv_Contact.Trim_FIRSTNAME & '"' & ' <' & Get_Bus_Serv_Contact.Trim_LONGEMAIL & '>'>
	</CFIF>
