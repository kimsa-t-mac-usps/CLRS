...
<!--- SAFEGUARD: Check if records already exist for NextReportDate to prevent duplicate loading --->
<CFQUERY NAME="CheckExistingRecords" DATASOURCE="ContLiab">
SELECT COUNT(*) AS EXISTING_COUNT
FROM CONTINGENT_LIAB_REPORT
WHERE DATE_REPORT = to_date('#DateFormat(NextReportDate, "mm/dd/yyyy")#', 'mm/dd/yyyy')
AND DELETED_FLAG IS NULL
</CFQUERY>

<CFIF CheckExistingRecords.EXISTING_COUNT GT 0>
<body>
<h2>Records Already Exist</h2>
<CFOUTPUT>
<p style="color:red; font-weight:bold">Cannot load records: #CheckExistingRecords.EXISTING_COUNT# records already exist for report date #DateFormat(NextReportDate, "mm/dd/yyyy")#.</p>
<p>Loading has been blocked to prevent duplicate data.</p>
<p><a href="Admin.full.cfm">Return to Admin page</a></p>
</CFOUTPUT>
</body>
</html>
<CFABORT>
